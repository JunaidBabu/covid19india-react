...
function LevelVaccinated2({data}) {
  const {t} = useTranslation();
  const spring = useSpring({
    total: getStatistic(data, 'total', 'seconddosevaccinated'),
        ...
        {spring.total.to(
          (total) =>
            `${formatNumber(Math.floor(total))} ${t(
              STATISTIC_CONFIGS['seconddosevaccinated'].displayName
            )}`
        )}
      </animated.h4>
    </div>
...
export default memo(LevelVaccinated2, isEqual);
