...
function LevelVaccinated1({data}) {
  const {t} = useTranslation();
  const spring = useSpring({
    total: getStatistic(data, 'total', 'firstdosevaccinated'),
        ...
        {spring.total.to(
          (total) =>
            `${formatNumber(Math.floor(total))} ${t(
              STATISTIC_CONFIGS['firstdosevaccinated'].displayName
            )}`
        )}
      </animated.h4>
    </div>
...
export default memo(LevelVaccinated1, isEqual);
